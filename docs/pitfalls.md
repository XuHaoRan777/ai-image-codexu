...
## 2026-07-04 AI 配置生成的文档抓取不能成为硬依赖

- 问题：AI 配置生成接收文档 URL 时，如果后端抓取公开文档失败就直接返回错误，会让用户无法继续使用辅助模型自身的 URL 读取能力，也无法在只提供 URL 的场景下软降级。
- 处理：文档抓取只能作为增强上下文。提示词中必须保留用户填写的文档地址；抓取成功时追加清洗后的正文，抓取失败时不拼正文、不抛出“文档抓取失败”，继续把 URL 和用户补充信息交给辅助模型。

## 2026-07-04 AI 配置生成不能只生成可落库空壳

- 问题：只要求辅助模型输出合法 JSON 和少量字段说明时，容易生成一条能插入数据库但缺少基础生图参数、参考图配置或返回提取路径的空壳配置，后续仍然无法直接用于生图。
- 处理：AI 配置生成提示词必须详细说明 `prompt`、`aspectRatio`、`resolution`、`quantity`、`referenceImages`、`extra`、`response` 和 `polling` 的生成规则。文档缺少尺寸、分辨率或数量个别字段时，应根据接口类型按 OpenAI-compatible 或 Google/Gemini-compatible 默认参数补齐；参考图必须至少生成 `mode=none,maxCount=16`，支持图生图时按文档选择 `inlineBase64` 或 `multipart`。
